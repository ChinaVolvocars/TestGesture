        ...
        mGestureContentView = new GestureContentView(this, false, "", new GestureDrawline.GestureCallBack() {
            ...
            public void onGestureCodeInput(String inputCode) {
                if (!isInputPassValidate(inputCode)) {
                    mTextTip.setText(Html.fromHtml("<font color='#DA3412'>至少链接4个点, 请重新绘制密码</font>"));
                ...
                if (mIsFirstInput) {
                    ...
                    mTextTip.setText("确认手势密码");
                } else {
                    if (inputCode.equals(mFirstPassword)) {
                        Toast.makeText(GestureEditActivity.this, "设置成功", Toast.LENGTH_SHORT).show();
                        mGestureContentView.clearDrawlineState(0L);
                        //在此处将用户输入的手势密码存起来
                        Log.d("蜡笔小新", inputCode);


                        GestureEditActivity.this.finish();
                    } else {
                        mTextTip.setText(Html.fromHtml("<font color='#DA3412'>与上一次绘制不一致，请重新绘制</font>"));
                        // 左右移动动画
